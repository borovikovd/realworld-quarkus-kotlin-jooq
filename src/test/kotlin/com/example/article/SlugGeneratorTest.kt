...
class SlugGeneratorTest {
    ...
    fun `should generate simple slug from title`() {
        val slug = SlugGenerator.generateUniqueSlug(title = "Hello World", existingSlugChecker = { false })
        assertEquals("hello-world", slug)
    }

    @Test
    fun `should handle special characters`() {
        val slug = SlugGenerator.generateUniqueSlug(title = "Hello! World? #Test", existingSlugChecker = { false })
        assertEquals("hello-world-test", slug)
    }

    @Test
    fun `should handle accented characters`() {
        val slug = SlugGenerator.generateUniqueSlug(title = "Café Français", existingSlugChecker = { false })
    ...
    fun `should generate unique slug with counter when slug exists`() {
        ...
        val slug = SlugGenerator.generateUniqueSlug(title = "Hello World", existingSlugChecker = { it in existingSlugs })
    ...
    fun `should handle titles with numbers`() {
        val slug = SlugGenerator.generateUniqueSlug(title = "Kotlin 2.0 Release", existingSlugChecker = { false })
        assertEquals("kotlin-20-release", slug)
    }

    @Test
    fun `should handle mixed case`() {
        val slug = SlugGenerator.generateUniqueSlug(title = "MiXeD CaSe TiTlE", existingSlugChecker = { false })
    ...
    fun `should throw exception when max attempts exhausted`() {
        val exception = assertThrows<IllegalStateException> {
            SlugGenerator.generateUniqueSlug(title = "Hello World", existingSlugChecker = { true })
        }
